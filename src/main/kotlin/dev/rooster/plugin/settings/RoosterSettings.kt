...
class RoosterState {
     ...
     * [dev.rooster.plugin.shell.ShellAllowListMatcher].
     */
    var shellAllowList: MutableList<String> =
        dev.rooster.plugin.shell.ShellAllowListMatcher.SAFE_DEFAULTS.toMutableList()
...
class RoosterSettings : PersistentStateComponent<RoosterState> {
    ...
    private var state = RoosterState()

    override fun getState(): RoosterState = state

    override fun loadState(loaded: RoosterState) {
    ...
    companion object {
        fun getInstance(): RoosterSettings =
            ApplicationManager.getApplication().getService(RoosterSettings::class.java)
    }
}
